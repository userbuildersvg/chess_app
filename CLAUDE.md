...
| `APP_MASTER_KEY` | — | **REQUIRED in production; the backend refuses to boot without it.** 32 bytes base64 (`tools/generate_master_key.py`). Wraps one random data key per account (`user_data_keys`, migration 014); imported PGNs, finding positions and durable-correction text are AES-GCM'd under it (`data_keys.py`). Deleting an account zeroes its wrapped key, which is what makes stale encrypted copies in backups unreadable. Losing this key makes every encrypted field unreadable - keep it in a password manager. Unset on a laptop = plaintext with a warning; `tools/encrypt_existing.py` seals rows written before the key existed |
| `PASSWORD_PEPPER` | — | optional secret HMAC'd into passwords before PBKDF2 (`password_security.py`). Old hashes keep verifying and are rehashed on the next login. Rotating it invalidates every hash made under the old value |
...
| file | what | needs |
|---|---|---|
| `test_account_security.py` | **55, the account security pass: PBKDF2-600k with the PASSWORD_PEPPER scheme marker, legacy hashes verifying and rehashing on login, the policy (8-256 chars, unicode, no whitespace-only), generic login refusal, cookie flags (HttpOnly, SameSite, Secure in production), hashed session tokens, logout, rate limit; AES-GCM sealing of PGN / finding position / correction text / evidence / practice position under per-account keys, guest rows unsealed, account B and guests cannot read A; deletion requires the password, ends sessions, hard-deletes and zeroes the data key so stale blobs are unreadable; production refuses to boot without APP_MASTER_KEY; disposable schema** | `DATABASE_URL` |
